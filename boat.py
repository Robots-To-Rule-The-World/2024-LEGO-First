    ...
    #-----------------------------------------------------------------------------------
    #This function needs work, for now use move_straight
    ...
    async def move_straight(distance, speed):
        await motor_pair.move_for_degrees(motor_pair.PAIR_1, degreesForDistance(distance),0,velocity=-1*speed, stop = motor.SMART_COAST)
    ...
    async def main():
        ...
        #start with commands for boat mission here
        await move_straight(55,1000) # (distance, speed) for distance 25 is roughly 1 foot
        await spin_turn_cw(180,400) # (degrees to turn, speed)
        await move_straight(55,600) # (distance, speed) for distance 25 is roughly 1 foot
        #await motor_pair.stop(motor_pair.PAIR_1,*,motor.motor.SMART_COAST)
        #await move_straight(20,300)
        #await motor.run_to_absolute_position(port.B,340,600)
        #await motor.run_to_absolute_position(port.B,260,400)
        #await move_straight(-8, 300)
        #await spin_turn_ccw(35,300)
        await spin_turn_ccw(180,400)
        
        #end command for boat mission     
